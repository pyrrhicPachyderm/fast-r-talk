...
\note[itemize]{
	...
	User and system time are complicated to explain; use elapsed.
	...
	I haven't included any output here, 'cause it's hard to come up with a small example where it actually demnstrates anything.
	Also, knitr makes a mess of it.
	
	\item
	There are several packages with alternatives.
	E.g.\ \texttt{microbenchmark} and Hadley Wickham's \texttt{profr}.
}
